...
class DataRetriever:
    ...
    def _score(
        self, doc: Document, q_tokens: set[str], focus: list[str]
    ) -> tuple[float, float]:
        ...
        content_boost = sum(
            self.score_boost_content_match for term in focus if term in content
        )
        source_penalty = (
            1.0 if source.endswith("index.txt") or source == "contents.txt" else 0.0
        )
