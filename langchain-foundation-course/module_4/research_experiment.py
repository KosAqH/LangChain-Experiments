...
# Compile
# Default graph for script/non-interactive runs.
graph = builder.compile()

# Optional graph for human-in-the-loop workflows.
interactive_graph = builder.compile(interrupt_before=['human_feedback'])

if __name__ == "__main__":
    state: ResearchGraphState = {"topic": "The future of AI assistants in the next 5 years",
                                 "max_analysts": 3,
                                 "human_analyst_feedback": "",
                                 "analysts": [],
                                 "sections": [],
    ...
    while True:
        interrupted_state = interactive_graph.invoke(state)

        analysts = interrupted_state.get("analysts", [])
        if analysts:
            print("\nGenerated analysts:")
        ...
        prompt = "\nType 'approve' to continue, or provide feedback to regenerate analysts: "
        user_feedback = input(prompt).strip()

        if user_feedback.lower() == "approve":
            state = {**interrupted_state, "human_analyst_feedback": "approve"}
            break

        # Any non-approve feedback is fed back into analyst generation.
        state = {**interrupted_state, "human_analyst_feedback": user_feedback}
